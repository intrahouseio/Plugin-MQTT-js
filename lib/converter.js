...
module.exports = {
  ...
  addPubMapItem(item) {
    if (item.id && item.topic && item.dn) {
      ...
      if (!this.pubMap.has(item.dn)) this.pubMap.set(item.dn, item);
    }
  },
  ...
  convertOutgoing(dn, val) {
    ...
    console.log("getTopicAndMessageFrompub 0");
    if (!this.pubMap.has(dn)) {
      // Пытаемся добавить, если значение пришло впервые???
      return;
    }

    console.log("getTopicAndMessageFrompub 1");
